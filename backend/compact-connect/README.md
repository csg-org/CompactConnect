...
domains and client IDs). A quick way to do that is to run `bin/fetch_aws_resources.py --as-env` and copy/paste the
output into `webroot/.env`. To see more data on your deployment in human-readable format (for example, DynamoDB table
names), run `bin/fetch_aws_resources.py` without any additional flags.
