...
class TstFunction(TstLambdas):
    ...
    def build_resources(self):
        ...
        self._table = boto3.resource('dynamodb').create_table(
            AttributeDefinitions=[
                ...
                {'AttributeName': 'ssn', 'AttributeType': 'S'},
                {'AttributeName': 'dateOfUpdate', 'AttributeType': 'S'},
                {'AttributeName': 'licenseHomeProviderId', 'AttributeType': 'S'},
                {'AttributeName': 'compactJur', 'AttributeType': 'S'},
                {'AttributeName': 'famGivMid', 'AttributeType': 'S'},
            ],
            ...
            GlobalSecondaryIndexes=[
                {
                    ...
                    'KeySchema': [
                        ...
                        {'AttributeName': 'licenseHomeProviderId', 'KeyType': 'RANGE'},
                    ],
                ...
                },
                {
                    ...
                    'KeySchema': [
                        {'AttributeName': 'compactJur', 'KeyType': 'HASH'},
                        {'AttributeName': 'famGivMid', 'KeyType': 'RANGE'},
                    ],
                ...
                },
                {
                    ...
                    'KeySchema': [
                        {'AttributeName': 'compactJur', 'KeyType': 'HASH'},
                        {'AttributeName': 'dateOfUpdate', 'KeyType': 'RANGE'},
                    ],
                    'Projection': {'ProjectionType': 'ALL'},
                },
            ],
    ...
    def _generate_licensees(self, home: str, privilege: str, start_serial: int):
        ...
        with open('tests/resources/api/license-post.json') as f:
            license_data = LicensePostSchema().load({'compact': 'aslp', 'jurisdiction': home, **json.load(f)})
        ...
        for i in range(start_serial, start_serial - 100, -1):
            ...
            privilege_data_copy.update({'providerId': provider_id, 'ssn': ssn, 'homeJurisdiction': home})
            item = PrivilegeRecordSchema().dump({'compact': 'aslp', 'jurisdiction': privilege, **privilege_data_copy})
