...
def _calculate_jurisdiction_fee(jurisdiction: Jurisdiction, user_active_military: bool, license_type_abbr: str) -> Decimal:
    """
    ...
    # If licenseFees is defined, look up the fee for the given license type
    license_fee = next((fee for fee in jurisdiction.license_fees if fee.license_type == license_type_abbr), None)
    if license_fee:
...
def _get_total_privilege_cost(
    compact: Compact, selected_jurisdictions: list[Jurisdiction], user_active_military: bool, license_type_abbreviation: str
) -> Decimal:
    """
    ...
    for jurisdiction in selected_jurisdictions:
        total_cost += _calculate_jurisdiction_fee(jurisdiction, user_active_military, license_type_abbreviation)
...
class AuthorizeNetPaymentProcessorClient(PaymentProcessorClient):
    ...
    def process_charge_on_credit_card_for_privilege_purchase(
        ...
        for jurisdiction in selected_jurisdictions:
            ...
            privilege_line_item.unitPrice = _calculate_jurisdiction_fee(jurisdiction, user_active_military, license_type_abbreviation)
