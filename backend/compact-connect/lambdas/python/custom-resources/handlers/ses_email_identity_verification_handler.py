...
def on_event(event: dict, context: LambdaContext):
    """
    CloudFormation event handler using the CDK provider framework.
    
    This custom resource verifies that a SES email identity is verified before allowing
    ...
    """
    logger.info(f"Entering SES email identity verification handler: {json.dumps(event)}")
