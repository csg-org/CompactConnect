...
class TestLicenses(TstFunction):
    ...
    def _post_license_with_previous_ssn(self) -> dict:
        """POST a single license carrying a previousSSN and return the raw response."""
        from handlers.licenses import post_licenses
        ...
        self.assertEqual(
            {
                ...
                'errors': {
                    '1': {
                        ...
                        # ssn is no longer a required field, so its absence is reported by the
                        # schema-level either-or check rather than as a missing required field. That
                        # check is skipped while any field-level errors are outstanding, which is why no
                        # ssn error appears here.
                    }
                },
            },
            json.loads(resp['body']),
...
class TestPostLicensesWithoutSsn(TstFunction):
    ...
    def test_returns_400_for_duplicate_license_number_and_license_type_in_one_request(self):
        ...
        self.assertEqual(
            [
                'Same license number for the same license type detected on multiple rows. Every record '
                'must have a unique license number per license type within the same request.'
            ],
    ...
    def test_accepts_the_same_license_number_for_two_license_types(self):
        ...
        """
        existing_license = self._seed_existing_license()
        self._seed_existing_license(licenseType='audiologist')

        resp = self._post(
            [
                self._license_without_ssn(licenseNumber=existing_license.licenseNumber),
                self._license_without_ssn(licenseNumber=existing_license.licenseNumber, licenseType='audiologist'),
            ]
        )
    ...
    def test_processes_a_mixed_batch_down_both_paths(self):
        ...
        license_with_ssn['licenseType'] = 'audiologist'
    ...
    def test_returns_400_when_neither_ssn_nor_license_number_is_provided(self):
        ...
        self.assertEqual(400, resp['statusCode'])
        body = json.loads(resp['body'])
        self.assertEqual({'0': {'ssn': ['ssn is required when licenseNumber is not provided.']}}, body['errors'])
