...
@mock_aws
class TestPrivilegeDeactivationMessageHandler(TstFunction):
    def _create_sqs_message(self):
        """Create a sample SQS event that would be sent to the handler"""
        message = {
            'eventTime': '2024-11-08T23:59:59+00:00',
            'compact': 'aslp',
            'jurisdiction': 'ne',
        ...
        # Return a properly formatted SQS event
        return {'Records': [{'messageId': '123', 'body': json.dumps(message)}]}

    @patch('cc_common.config._Config.current_standard_datetime', datetime.fromisoformat('2024-11-08T23:59:59+00:00'))
    @patch('cc_common.config._Config.email_service_client')
    def test_privilege_deactivation_message_handler_sends_expected_email_notifications(self, mock_email_service_client):
        """
        The privilege deactivation message handler should send email notifications to the provider and jurisdiction
        """
        from handlers.privileges import privilege_deactivation_message_handler
        
        self._load_provider_data()
        
        event = self._create_sqs_message()
        response = privilege_deactivation_message_handler(event, self.mock_context)
        
        # Verify no batch failures were reported
    ...
    @patch('cc_common.config._Config.current_standard_datetime', datetime.fromisoformat('2024-11-08T23:59:59+00:00'))
    @patch('cc_common.config._Config.email_service_client')
    def test_reports_batch_failure_when_provider_notification_fails(self, mock_email_service_client):
        """
        If the deactivation notification to the provider fails to send, the handler should report a batch failure
        so that the message can be retried according to the queue's retry policy.
        """
        from handlers.privileges import privilege_deactivation_message_handler
        from cc_common.exceptions import CCInternalException
        ...
        # Verify a batch failure was reported for the message
        self.assertEqual({'batchItemFailures': [{'itemIdentifier': '123'}]}, response)
        
        # Verify the provider notification attempt was made
        mock_email_service_client.send_provider_privilege_deactivation_email.assert_called_once_with(
            compact='aslp',
            provider_email='björkRegisteredEmail@example.com',
            privilege_id='SLP-NE-1',
    ...
    @patch('cc_common.config._Config.current_standard_datetime', datetime.fromisoformat('2024-11-08T23:59:59+00:00'))
    @patch('cc_common.config._Config.email_service_client')
    def test_reports_batch_failure_when_jurisdiction_notification_fails(self, mock_email_service_client):
        """
        If the deactivation notification to the jurisdiction fails to send, the handler should report a batch failure
        so that the message can be retried according to the queue's retry policy.
        """
        from handlers.privileges import privilege_deactivation_message_handler
        from cc_common.exceptions import CCInternalException
