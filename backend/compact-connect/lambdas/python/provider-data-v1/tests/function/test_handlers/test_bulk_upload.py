...
class TestBulkUploadWithoutSsn(TstFunction):
    ...
    CSV_HEADER = (
        ...
        'homeAddressPostalCode,licenseType'
    )
    ...
    def _csv_row(
        ...
        *,
        ...
        license_type: str = 'speech-language pathologist',
        family_name: str = 'Guðmundsdóttir',
    ) -> str:
        return (
            f'{ssn},{license_number},Björk,{family_name},1985-06-06,2010-06-06,2050-04-04,'
            f'active,eligible,123 A St.,Columbus,oh,43004,{license_type}'
        )
    ...
    def test_emits_validation_error_for_an_ambiguous_license_number_and_keeps_processing(self):
        ...
        other_license = self._seed_existing_license(licenseNumber='B0608337260', licenseType='audiologist')

        _, entries = self._process_csv(
            [
                self._csv_row(license_number=existing_license.licenseNumber),
                self._csv_row(license_number=other_license.licenseNumber, license_type='audiologist'),
            ]
        )
    ...
    def test_emits_validation_error_for_a_duplicate_license_number_in_the_same_file(self):
        ...
        self.assertEqual(
            [
                ...
                'matches with record 1. Every record must have a unique license number per license type '
                'within the same request.'
            ],
            validation_errors[0]['errors']['_schema'],
        )
    ...
    def test_accepts_the_same_license_number_for_two_license_types(self):
        ...
        _, entries = self._process_csv(
            [
                self._csv_row(license_number=existing_license.licenseNumber),
                self._csv_row(license_number=existing_license.licenseNumber, license_type='audiologist'),
            ]
        )
    ...
    def test_processes_a_mixed_file_down_both_paths(self):
        ...
        _, entries = self._process_csv(
            [
                self._csv_row(ssn='123-45-6789', license_number='C0608337260', license_type='audiologist'),
                self._csv_row(license_number=existing_license.licenseNumber),
            ]
        )
