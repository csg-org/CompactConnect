...
class TestProviderRecordUtilityActiveSinceCalculation(TstLambdas):
    ...
    def test_calculation_returns_none_if_license_not_active(self):
        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2025-04-04')}
        )
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license, license_updates=[]
        )

        self.assertEqual(None, active_since)

    def test_calculation_returns_issuance_date_if_no_license_deactivation_events(self):
        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2100-04-04')}
        )
        active_since = self.test_model.calculate_license_active_since_date(
        ...
        self.assertEqual(test_license.dateOfIssuance, active_since)

    def test_calculation_returns_renewal_date_if_license_expired_and_was_then_renewed(self):
        from cc_common.data_model.schema.common import UpdateCategory

        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2100-04-04')}
        )
        # this simulates the scenario where a license record expired before renewal.
        # the privilege is then renewed some time later.
        test_expiration_event = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'updateType': UpdateCategory.EXPIRATION,
                'effectiveDate': datetime.fromisoformat('2045-04-04T12:59:59+00:00'),
            }
        )
        # The default license update record generated by the test data generator is a renewal event
        test_renewal_update = self.test_data_generator.generate_default_license_update(
            value_overrides={'effectiveDate': datetime.fromisoformat('2099-04-04T12:59:59+00:00')}
        )
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license, license_updates=[test_expiration_event, test_renewal_update]
        )

        self.assertEqual(test_renewal_update.updatedValues.get('dateOfRenewal'), active_since)

    def test_calculation_returns_renewal_date_if_license_deactivated_and_was_then_renewed(self):
        from cc_common.data_model.schema.common import UpdateCategory

        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2100-04-04')}
        )
        # this simulates the scenario where a license record was deactivated by a state_upload admin.
        # the privilege is then renewed some time later.
        test_deactivation_event = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'updateType': UpdateCategory.DEACTIVATION,
                'effectiveDate': datetime.fromisoformat('2045-04-04T12:59:59+00:00'),
            }
        ...
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license, license_updates=[test_deactivation_event, test_renewal_update]
        )

        self.assertEqual(test_renewal_update.updatedValues.get('dateOfRenewal'), active_since)

    def test_calculation_returns_renewal_date_if_license_was_encumbered_and_then_renewed(self):
        from cc_common.data_model.schema.common import UpdateCategory

        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2100-04-04')}
        )
        # this simulates the scenario where a license record was encumbered.
        # the privilege is then renewed some time later after the having the encumbrance lifted.
        test_encumbrance_event = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'updateType': UpdateCategory.ENCUMBRANCE,
                'effectiveDate': datetime.fromisoformat('2045-04-04T12:59:59+00:00'),
            }
        )
        test_encumbrance_lifting_event = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'updateType': UpdateCategory.LIFTING_ENCUMBRANCE,
                'effectiveDate': datetime.fromisoformat('2047-04-04T12:59:59+00:00'),
            }
        ...
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license,
            license_updates=[test_encumbrance_event, test_encumbrance_lifting_event, test_renewal_update],
        )

        self.assertEqual(test_renewal_update.updatedValues.get('dateOfRenewal'), active_since)

    def test_calculation_returns_issued_date_if_license_renewed_before_expiration(self):
        test_license = self.test_data_generator.generate_default_license(
            value_overrides={'dateOfExpiration': date.fromisoformat('2100-04-04')}
        )
        # this simulates the scenario where a license record was renewed before it expired.
        test_renewal_update = self.test_data_generator.generate_default_license_update(
            value_overrides={'effectiveDate': datetime.fromisoformat('2099-04-04T12:59:59+00:00')}
        )
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license, license_updates=[test_renewal_update]
        )

        self.assertEqual(test_license.dateOfIssuance, active_since)

    def test_calculation_returns_oldest_renewal_date_if_license_expired_and_then_was_renewed_multiple_times(self):
        from cc_common.data_model.schema.common import UpdateCategory
        ...
        # this simulates the scenario where a license record expired before renewal.
        # the license is then renewed some time later, and then renewed again before expiration
        # the oldest renewal date should be returned in this case
        test_expiration_event = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'updateType': UpdateCategory.EXPIRATION,
        ...
        # The default privilege update record generated by the test data generator is a renewal event
        test_first_renewal_update = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'effectiveDate': datetime.fromisoformat('2098-04-04T12:59:59+00:00'),
                'updatedValues': {'dateOfRenewal': datetime.fromisoformat('2098-04-04')},
            }
        )
        # The default privilege update record generated by the test data generator is a renewal event
        test_second_renewal_update = self.test_data_generator.generate_default_license_update(
            value_overrides={
                'effectiveDate': datetime.fromisoformat('2099-04-04T12:59:59+00:00'),
                'updatedValues': {'dateOfRenewal': datetime.fromisoformat('2099-04-04')},
            }
        )
        active_since = self.test_model.calculate_license_active_since_date(
            license_record=test_license,
            license_updates=[test_expiration_event, test_first_renewal_update, test_second_renewal_update],
        )

        self.assertEqual(date.fromisoformat('2098-04-04'), active_since)
