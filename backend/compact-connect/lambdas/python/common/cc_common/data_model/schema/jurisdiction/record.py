...
class JurisdictionPrivilegeFeeRecordSchema(Schema):
    ...
    amount = PositiveDecimal(required=True, allow_none=False, places=2)
    militaryRate = PositiveDecimal(required=False, allow_none=True, places=2)
...
class JurisdictionRecordSchema(BaseRecordSchema):
    ...
    def generate_pk_sk(self, in_data, **kwargs):  # noqa: ARG001 unused-argument
        """
        Generates and assigns primary and sort keys for a jurisdiction configuration record.
        
        The primary key (`pk`) is set to the compact identifier followed by '#CONFIGURATION'.
        The sort key (`sk`) is set to the compact identifier, '#JURISDICTION#', and the lowercase postal abbreviation.
        """
    ...
    def validate_privilege_fees(self, data, **kwargs):  # noqa: ARG001 unused-argument
        """
        Validates the privilegeFees list for correct license type abbreviations and completeness.
        
        Ensures that all license type abbreviations in privilegeFees are valid for the specified compact, that there are no duplicates, and that all required license types for the compact are present. Raises a ValidationError with detailed messages if any of these conditions are not met.
        """
