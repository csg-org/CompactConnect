...
# Searches for adverse actions that have NOT been lifted (no liftingUser field)
QUERY_PROVIDERS_WITH_ENCUMBRANCE = {
    "query": {
        "bool": {
            "should": [
                # Check for non-lifted adverse actions in nested licenses
                {
                    "nested": {
                        "path": "licenses.adverseActions",
                        "query": {
                            "bool": {
                                "must_not": {
                                    "exists": {"field": "licenses.adverseActions.liftingUser"}
                                }
                            }
                        }
                    }
                },
                # Check for non-lifted adverse actions in nested privileges
                {
                    "nested": {
                        "path": "privileges.adverseActions",
                        "query": {
                            "bool": {
                                "must_not": {
                                    "exists": {"field": "privileges.adverseActions.liftingUser"}
                                }
                            }
                        }
                    }
                    ...
                    "adverseActions": [
                        {
                            ...
                            "effectiveStartDate": (now - timedelta(days=150)).date().isoformat(),
                            "creationDate": (now - timedelta(days=150)).isoformat(),
                            "adverseActionId": "aa-002",
                            "effectiveLiftDate": (now - timedelta(days=60)).date().isoformat(),
                            "dateOfUpdate": (now - timedelta(days=60)).isoformat(),
                            "encumbranceType": "practice-restriction",
                            "clinicalPrivilegeActionCategories": ["geriatrics"],
                            "submittingUser": "staff-user-456",
                            "liftingUser": "staff-user-789"  # This adverse action has been LIFTED
                        }
                    ],
                    "investigations": [],
                    ...
                    "adverseActions": [
                        {
                            ...
                            "licenseType": "speech-language-pathologist",
                            "actionAgainst": "privilege",
                            "effectiveStartDate": (now - timedelta(days=30)).date().isoformat(),
                            "creationDate": (now - timedelta(days=30)).isoformat(),
                            "adverseActionId": "aa-001",
                            "dateOfUpdate": (now - timedelta(days=30)).isoformat(),
                            "encumbranceType": "practice-restriction",
                            "clinicalPrivilegeActionCategories": ["pediatrics"],
                            "submittingUser": "staff-user-123"
                            # Note: NO liftingUser field - this adverse action is ACTIVE
                        }
                    ],
                    "investigations": [],
                    "administratorSetStatus": "active",
