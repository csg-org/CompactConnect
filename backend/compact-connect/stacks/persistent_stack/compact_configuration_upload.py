...
class CompactConfigurationUpload(Construct):
    ...
    def _validate_jurisdiction_configuration(self, compact: str, jurisdiction: dict, license_types_for_compact: list[dict]):
        ...
        if jurisdiction.get('licenseFees'):
            defined_license_types = [fee['licenseType'] for fee in jurisdiction['licenseFees']]
            missing_license_types = set(license_type_abbreviations) - set(defined_license_types)
            
            if missing_license_types:
                raise ValueError(
                    f'Jurisdiction {jurisdiction["postalAbbreviation"]} is missing license fees for the following license types: '
                    f'{", ".join(missing_license_types)}'
                )
            
            # Check for duplicate license types
            if len(defined_license_types) != len(set(defined_license_types)):
                raise ValueError(
                    f'Jurisdiction {jurisdiction["postalAbbreviation"]} has duplicate license type fees'
                )
            ...
            # Neither licenseFees nor jurisdictionFee is defined
            raise ValueError(
                f'Jurisdiction {jurisdiction["postalAbbreviation"]} in compact {compact} must define licenseFees'
            )
