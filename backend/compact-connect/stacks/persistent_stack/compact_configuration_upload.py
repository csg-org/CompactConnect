...
class CompactConfigurationUpload(Construct):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes a custom resource construct to upload attestation configuration data to a DynamoDB table.
        
        Creates a Lambda-backed custom resource that reads and validates attestation data from a YAML file, then uploads it to the specified DynamoDB table with appropriate permissions and security suppressions. The construct configures the Lambda function, grants necessary access to the DynamoDB table and KMS key, applies compliance suppressions, and passes active compact member jurisdictions and attestations as properties to the custom resource.
        """
    ...
    def _read_attestations_file(self) -> list:
        """
        Reads and validates attestation data from the 'compact-config/attestations.yml' file.
        
        Parses the YAML file, verifies its structure, and ensures each attestation entry contains all required fields with correct types. Raises a RuntimeError if the file is missing, malformed, or contains invalid data.
        
        Returns:
            A list of validated attestation dictionaries.
        """
