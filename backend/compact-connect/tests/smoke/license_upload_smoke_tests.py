...
def upload_licenses_record():
    ...
    for _ in range(15):
        ssn_record_response = provider_dynamo_table.get_item(
            Key={
                'pk': f'{COMPACT}#SSN#{MOCK_SSN}',
                'sk': f'{COMPACT}#SSN#{MOCK_SSN}'
            }
        )
    ...
    assert 'Item' in ssn_record_response, (f'Failed to find license record in provider table. '
                                           f'Response: {ssn_record_response}')
    ...
    license_record = next((record for record in provider_record_query_response['Items']
                           if record['type'] == 'license'), None)
    ...
    provider_record = next((record for record in provider_record_query_response['Items']
                            if record['type'] == 'provider'), None)
    ...
    license_ingest_record_response = data_events_table.query(
        ...
        ExpressionAttributeValues={
            ':pk': f'COMPACT#aslp#JURISDICTION#ne',
    ...
    assert license_ingest_record_response.get('Items'), (f'Failed to find license ingest record in data events table.'
                                                         f' Response: {license_ingest_record_response}')
    print(f'License ingest data event successfully added to data events table {license_ingest_record_response["Items"]}')

    # now clear out the records we added
    provider_dynamo_table.delete_item(
        Key={
            'pk': f'{COMPACT}#SSN#{MOCK_SSN}',
            'sk': f'{COMPACT}#SSN#{MOCK_SSN}'
        }
