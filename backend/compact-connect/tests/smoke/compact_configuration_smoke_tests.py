...
def test_active_member_jurisdictions():
    """
    Verifies that active member jurisdictions from the local cdk.json file match those returned by the API for each compact.
    
    Raises:
        SmokeTestFailureException: If the cdk.json file is missing required data, the API call fails, or the jurisdictions do not match.
    """
...
def test_compact_configuration():
    """
    Updates and verifies the compact configuration as a compact admin user.
    
    Creates a temporary compact admin user, submits a new configuration for the first compact, retrieves it, and checks that the API response matches the submitted configuration (including API-added fields). Cleans up the test user after execution.
    
    Returns:
        dict: The compact configuration as returned by the API.
    
    Raises:
        SmokeTestFailureException: If the API request fails or the configuration does not match.
    """
...
def test_jurisdiction_configuration():
    """
    Tests that a state admin user can update and retrieve jurisdiction configuration via the API.
    
    Creates a temporary state admin user for a specific jurisdiction, submits a configuration update, retrieves the configuration, and verifies that the API response matches the expected data. Cleans up the test user after execution.
    
    Raises:
        SmokeTestFailureException: If any API call fails or if the configuration does not match the expected values.
    
    Returns:
        dict: The jurisdiction configuration as returned by the API.
    """
