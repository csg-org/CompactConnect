...
def test_home_jurisdiction_change_inactivates_privileges_when_no_license_in_new_jurisdiction():
    ...
    if provider_info_after_purchase.get('compactEligibility') != 'eligible':
        raise SmokeTestFailureException(
            f"Provider should be 'eligible' before home jurisdiction change, but got '{provider_info_after_purchase.get('compactEligibility')}'"
        )
    ...
    ne_privilege_after_change = next(
        (privilege for privilege in privileges_after_change if privilege['jurisdiction'] == 'ne'), 
        None
    )
    ...
    if response.status_code != 200:
        raise SmokeTestFailureException(f"Failed to restore original home jurisdiction: {response.status_code}: {response.text}")
    ...
    ne_privilege_after_reactivation = next(
        (privilege for privilege in privileges_after_reactivation if privilege['jurisdiction'] == 'ne'),
        None
    )
    if not ne_privilege_after_reactivation:
        raise SmokeTestFailureException("Nebraska privilege not found after reactivation")
    if (ne_privilege_after_reactivation.get('status') != 'active'
