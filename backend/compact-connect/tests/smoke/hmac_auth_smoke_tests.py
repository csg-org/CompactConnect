...
COMPACT = 'aslp'
JURISDICTION = 'ne'
TEST_CLIENT_NAME = 'test-hmac-auth-client'
TEST_KEY_ID = 'test-hmac-key-001'

# This script can be run locally to test the HMAC authentication flow against a sandbox environment
...
def create_test_app_client():
    """
    Create a test app client in Cognito for HMAC authentication testing.
...
def configure_hmac_public_key(public_key_pem: str):
    """
    Configure an HMAC public key in the compact configuration table.

    :param public_key_pem: PEM-encoded public key content
    """
    logger.info(f'Configuring HMAC public key: {TEST_KEY_ID}')
    ...
    try:
        ...
        if 'Item' in response:
            logger.info(f'HMAC key {TEST_KEY_ID} already exists, overwriting')
        ...
        logger.info(f'Successfully configured HMAC public key: {TEST_KEY_ID}')

    except ClientError as e:
        logger.error(f'Failed to configure HMAC public key: {str(e)}')
        raise SmokeTestFailureException(f'Failed to configure HMAC public key: {str(e)}') from e


def remove_hmac_public_key():
    """Remove the test HMAC public key from the compact configuration table."""
    try:
        table = get_compact_configuration_table()
        ...
        logger.info(f'Successfully removed HMAC public key: {TEST_KEY_ID}')

    except ClientError as e:
        logger.error(f'Failed to remove HMAC public key: {str(e)}')
        # Don't raise here as this is cleanup
...
def create_signed_headers(method: str, path: str, query_params: dict, private_key_pem: str):
    """
    Create HMAC-signed headers for a request.
...
def test_bulk_upload_endpoint_without_hmac(client_id: str, client_secret: str):
    """
    Test the bulk-upload endpoint without HMAC authentication (should succeed when no keys configured).

    :param client_id: The client ID for authentication
    :param client_secret: The client secret for authentication
    """
    logger.info('Testing bulk-upload endpoint without HMAC authentication')
    ...
    if response.status_code != 200:
        raise SmokeTestFailureException(
            f'Bulk-upload endpoint should succeed without HMAC when no keys configured. '
            f'Response: {response.status_code} - {response.text}'
        )

    logger.info('Bulk-upload endpoint succeeded without HMAC authentication (as expected)')


def test_bulk_upload_endpoint_without_hmac_after_key_configuration(client_id: str, client_secret: str):
    """
    Test the bulk-upload endpoint without HMAC authentication after keys are configured (should fail).

    :param client_id: The client ID for authentication
    :param client_secret: The client secret for authentication
    """
    logger.info('Testing bulk-upload endpoint without HMAC authentication after key configuration')
    ...
    if response.status_code == 200:
        raise SmokeTestFailureException('Bulk-upload endpoint should fail without HMAC when keys are configured')

    if response.status_code != 401:
        logger.warning(f'Expected 401 but got {response.status_code}: {response.text}')

    logger.info('Bulk-upload endpoint correctly rejected without HMAC authentication')


def test_bulk_upload_endpoint_with_hmac(client_id: str, client_secret: str, private_key_pem: str):
    """
    Test the bulk-upload endpoint with valid HMAC authentication.

    :param client_id: The client ID for authentication
    :param client_secret: The client secret for authentication
    :param private_key_pem: PEM-encoded private key for signing
    """
    logger.info('Testing bulk-upload endpoint with HMAC authentication')

    # Get client credentials headers
    client_headers = get_client_auth_headers(client_id, client_secret)

    # Create HMAC headers
    hmac_headers = create_signed_headers(
        method='GET',
    ...
    # Combine headers
    headers = {**client_headers, **hmac_headers}
    ...
    if response.status_code != 200:
        raise SmokeTestFailureException(
            f'Bulk-upload endpoint should succeed with valid HMAC authentication. '
            f'Response: {response.status_code} - {response.text}'
        )
    ...
    logger.info('Bulk-upload endpoint succeeded with HMAC authentication')


def test_providers_query_endpoint_with_hmac(client_id: str, client_secret: str, private_key_pem: str):
    """
    Test the providers/query endpoint with valid HMAC authentication.

    :param client_id: The client ID for authentication
    :param client_secret: The client secret for authentication
    :param private_key_pem: PEM-encoded private key for signing
    """
    logger.info('Testing providers/query endpoint with HMAC authentication')
    ...
    if response.status_code != 200:
        raise SmokeTestFailureException(
            f'Providers/query endpoint should succeed with valid HMAC authentication. '
            f'Response: {response.status_code} - {response.text}'
        )
    ...
    logger.info('Providers/query endpoint succeeded with HMAC authentication')


def hmac_authentication_smoke_test():
    """
    Comprehensive smoke test for HMAC authentication system.

    This test exercises the HMAC authentication by:
    1. Creating a test app client in Cognito
    2. Testing bulk-upload endpoint without HMAC (should succeed when no keys configured)
    3. Configuring an HMAC public key for the test compact/state
    4. Testing bulk-upload endpoint without HMAC (keys configured - should fail)
    5. Testing bulk-upload endpoint with valid HMAC authentication
    6. Testing providers/query endpoint with valid HMAC authentication
    """
    logger.info('Starting HMAC authentication smoke test')
    ...
    try:
        # Step 1: Test bulk-upload endpoint without HMAC (no keys configured)
        test_bulk_upload_endpoint_without_hmac(client_id, client_secret)

        # Step 2: Configure HMAC public key
        configure_hmac_public_key(public_key_pem)

        # Step 3: Test bulk-upload endpoint without HMAC (keys configured - should fail)
        test_bulk_upload_endpoint_without_hmac_after_key_configuration(client_id, client_secret)

        # Step 4: Test bulk-upload endpoint with valid HMAC authentication
        test_bulk_upload_endpoint_with_hmac(client_id, client_secret, private_key_pem)

        # Step 5: Test providers/query endpoint with valid HMAC authentication
        test_providers_query_endpoint_with_hmac(client_id, client_secret, private_key_pem)

        logger.info('HMAC authentication smoke test completed successfully')

    finally:
        # Cleanup
        logger.info('Cleaning up test resources')
        remove_hmac_public_key()
...
if __name__ == '__main__':
    ...
    try:
        hmac_authentication_smoke_test()
        logger.info('HMAC authentication smoke test passed')
    except SmokeTestFailureException as e:
        logger.error(f'HMAC authentication smoke test failed: {str(e)}')
        raise
