...
class SSNTable(Table):
    ...
    def _setup_license_preprocessor_queue(self, data_event_bus: EventBus, alarm_topic: ITopic):
        ...
        preprocess_handler = PythonFunction(
            ...
            # A previousSSN correction makes this handler resolve two SSNs per license rather than one, and a
            # batch may hold up to batch_size of them, so it needs more headroom than a plain preprocess batch
