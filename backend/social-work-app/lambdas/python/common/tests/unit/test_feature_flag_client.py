...
class TestFeatureFlagClient(TstLambdas):
    ...
    def test_is_feature_enabled_with_context(self):
        ...
        context = FeatureFlagContext(user_id='user123', custom_attributes={'licenseType': 'lcsw'})
        ...
        with patch('cc_common.feature_flag_client.requests.post', return_value=mock_response) as mock_post:
            ...
            mock_post.assert_called_once_with(
                'https://api.example.com/v1/flags/test-flag/check',
                json={
                    'context': {'userId': 'user123', 'customAttributes': {'licenseType': 'lcsw'}},
                },
                timeout=5,
                headers={'Content-Type': 'application/json'},
            )
    ...
    def test_feature_flag_context_with_custom_attributes_only(self):
        ...
        context = FeatureFlagContext(custom_attributes={'licenseType': 'lcsw', 'jurisdiction': 'oh'})
        result = context.to_dict()

        self.assertEqual(result, {'customAttributes': {'licenseType': 'lcsw', 'jurisdiction': 'oh'}})
