...
class TestPostPrivilegeInvestigation(TstFunction):
    ...
    def _when_testing_privilege_investigation(self):
        ...
        test_event = self.test_data_generator.generate_test_api_event(
            ...
            scope_override=f'openid email {DEFAULT_PRIVILEGE_JURISDICTION}/socw.admin',
            value_overrides={
                'httpMethod': 'POST',
                ...
                'pathParameters': {
                    ...
                    'licenseType': license_record.licenseTypeAbbreviation,
                },
            },
        )

        # return both the test event and the test privilege record
        return test_event, license_record
    ...
    def test_privilege_investigation_handler(self, mock_publish_event):
        ...
        event, test_license_record = self._when_testing_privilege_investigation()
        ...
        expected_investigation = {
            ...
            'homeJurisdictionAtTimeOfCreation': DEFAULT_LICENSE_JURISDICTION,
        ...
        # Verify that investigation objects are included in the API response (from investigation records only)
        api_event = self.test_data_generator.generate_test_api_event(
            scope_override=f'openid email {DEFAULT_PRIVILEGE_JURISDICTION}/socw.readGeneral',
            value_overrides={
                'httpMethod': 'GET',
...
class TestPostLicenseInvestigation(TstFunction):
    ...
    def test_license_investigation_handler(self, mock_publish_event):
        ...
        api_event = self.test_data_generator.generate_test_api_event(
            scope_override=f'openid email {test_license_record.jurisdiction}/socw.readGeneral',
            value_overrides={
                'httpMethod': 'GET',
                'resource': '/v1/compacts/{compact}/providers/{providerId}',
                'pathParameters': {
...
class TestPatchPrivilegeInvestigationClose(TstFunction):
    ...
    def test_privilege_investigation_close_handler(self, mock_publish_event):
        ...
        event, test_license_record, investigation_id = self._when_testing_privilege_investigation_close()
        ...
        # Verify that investigation objects are removed from the API response (from investigation records only)
        api_event = self.test_data_generator.generate_test_api_event(
            scope_override=f'openid email {DEFAULT_PRIVILEGE_JURISDICTION}/socw.readGeneral',
            value_overrides={
                'httpMethod': 'GET',
