...
class TestLicenseNumberDedupeKey(TstLambdas):
    def test_key_distinguishes_license_type(self):
        ...
        self.assertNotEqual(
            license_number_dedupe_key(_license(licenseType='licensed clinical social worker')),
            license_number_dedupe_key(_license(licenseType='licensed master social worker')),
        )
    ...
    def test_key_distinguishes_license_scope(self):
        """Rows differing only in scope write different license records, so they are not duplicates."""
        from license_upload_without_ssn import license_number_dedupe_key

        self.assertNotEqual(
            license_number_dedupe_key(_license(licenseScope='single-state')),
            license_number_dedupe_key(_license(licenseScope='multi-state')),
        )
...
class TestResolveLicenseWithoutSsn(TstLambdas):
    ...
    def test_allows_the_same_license_number_for_a_different_license_type(self, mock_config):
        ...
        _resolve(_license(), record_position=1, seen_license_keys=seen_license_keys)
        resolved = _resolve(
            _license(licenseType='licensed master social worker'),
            record_position=2,
