...
2. **Retrieve the DynamoDB Record**
   ...
   - Search for an item where:
     ...
     - `sk` = `COMPACT#{compact}` (where `compact` is the user's compact, e.g., "socw")
...
In the DynamoDB console, perform the following steps:
   ...
   - Click the 'Save and close' button to archive the record
   - Verify the archived record was updated successfully
