...
first time a license is ingested for a practitioner (see [License Preprocessing](#ingest-flow)). The `providerId`
uses a [version 4 UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier) (UUID4). The RID
(`providerId`) is:
