...
- For CSV uploads, license numbers must be unique per license type and scope within a single file, and for JSON uploads unique per license type and scope within a single request. This applies to records uploaded without an SSN.
