        ...
        *,
        ...
        with self.subTest(provider_users_stack.stack_name):
            ...
            if domain_name is not None:
                callbacks.append(f'https://{domain_name}/auth/callback/licensee/psypact')
                callbacks.append(f'https://{domain_name}/auth/callback')
            if allow_local_ui:
                # 3018 is default
                local_ui_port = '3018' if not local_ui_port else local_ui_port
                callbacks.append(f'http://localhost:{local_ui_port}/auth/callback/licensee/psypact')
    ...
    def _inspect_persistent_stack(
        ...
        *,
        ...
        with self.subTest(persistent_stack.stack_name):
            ...
            callbacks = []
            if domain_name is not None:
                callbacks.append(f'https://{domain_name}/auth/callback/staff/psypact')
                callbacks.append(f'https://{domain_name}/auth/callback')
            if allow_local_ui:
                # 3018 is default
                local_ui_port = '3018' if not local_ui_port else local_ui_port
                callbacks.append(f'http://localhost:{local_ui_port}/auth/callback/staff/psypact')
                callbacks.append(f'http://localhost:{local_ui_port}/auth/callback')
