...
class TestBulkUploadWithoutSsn(TstFunction):
    ...
    def test_emits_validation_error_for_an_ambiguous_license_number_and_keeps_processing(self):
        ...
        other_license = self._seed_existing_license(licenseNumber='B0608337260', licenseType='esthetician')

        _, entries = self._process_csv(
            [
                self._csv_row(license_number=existing_license.licenseNumber),
                self._csv_row(license_number=other_license.licenseNumber, license_type='esthetician'),
            ]
        )
    ...
    def test_accepts_the_same_license_number_for_two_license_types(self):
        ...
        _, entries = self._process_csv(
            [
                self._csv_row(license_number=existing_license.licenseNumber),
                self._csv_row(license_number=existing_license.licenseNumber, license_type='esthetician'),
            ]
        )
    ...
    def test_processes_a_mixed_file_down_both_paths(self):
        ...
        _, entries = self._process_csv(
            [
                self._csv_row(ssn='123-45-6789', license_number='C0608337260', license_type='esthetician'),
                self._csv_row(license_number=existing_license.licenseNumber),
            ]
        )
