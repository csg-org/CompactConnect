...
def license_number_dedupe_key(license_record: dict) -> tuple:
    ...
    This mirrors the existing SSN duplicate rule: a license number may appear once per license type,
    because a jurisdiction may hold the same number across license types for one practitioner, but the
    same number and license type twice in one upload is a clerical error we reject rather than
    processing both rows against the same license record.
    """
