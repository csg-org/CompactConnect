...
The scan no longer fails silently: the `exitStatus` job **fails the build on any High-risk alert** and reports Medium+ (Mediums do not fail the job — known false positives like SQL injection, ruleId 40018, are filtered globally and do not count). Both an HTML report and a machine-readable `traditional-json` report (`zap-report.json`) are emitted and uploaded as the **ZAP Report** artifact. Findings are triaged via a GitHub issue labelled `security` for review.
