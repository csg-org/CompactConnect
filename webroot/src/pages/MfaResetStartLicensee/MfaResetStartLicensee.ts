...
import { AuthTypes, getHostedLoginUri, createAuthCsrfState } from '@utils/auth';
...
class MfaResetStartLicensee extends mixins(MixinForm) {
    //
    // Data
    //
    isFinalError = false;
    ...
    get hostedForgotPasswordUriLicensee(): string {
        return getHostedLoginUri(this.appMode, AuthTypes.LICENSEE, '/forgotPassword', this.csrfState);
    }
    ...
    initExtraFields(): void { // See Auth -> Registration section of README
        const reenterPasswordInput: HTMLElement = this.$refs.reenterPassword as HTMLElement;

        if (reenterPasswordInput) {
            reenterPasswordInput.style.display = 'inline-block';
            reenterPasswordInput.style.height = '1px';
            reenterPasswordInput.style.width = '1px';
            reenterPasswordInput.style.overflow = 'hidden';
        }
    }
    ...
    async handleSubmit(): Promise<void> {
        ...
        if (this.isFormValid) {
            ...
            const reenterPassword = document.getElementById('reenter-password') as HTMLInputElement;
...
export default toNative(MfaResetStartLicensee);

// export default MfaResetStartLicensee;
